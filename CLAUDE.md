...
- **Phase 9 (Aug 22, "The Escalation Ladder")**: the latency phase. A 21.8s clip took ~20 minutes — sixteen Parallel Task runs, six returning no owner, seven of them human faces. Fixed by *routing*, not by detecting less (recall is the whole safety claim).
  ...
  - Demo ledger 12 → 21 grants across 8 gap archetypes; `docs/sample-rights-ledger.csv` 5 → 14 rows. Demo scene e1 relabelled to the description the model actually produced, so demo mode shows the promotion end to end. **231 tests, smoke green.**
...
NOT DONE:
- **Two-phase reporting.** The complete timestamped risk report (scan + fingerprint + corroborate + triage + provisional risk + territory) could be emitted at ~45s over the existing SSE channel while DEEP ownership research enriches it in the background. `scoring.provisional_score` exists for exactly this; the emit is not wired. This is the remaining perceived-latency win.
...
- Tests: `.venv/bin/pytest -q` (231) · Smoke: `bash scripts/smoke.sh` · Frontend: `cd webapp && npm run build`
