...
- **Phase 9 (Aug 22, "The Escalation Ladder")**: the latency phase. A 21.8s clip took ~20 minutes — sixteen Parallel Task runs, six returning no owner, seven of them human faces. Fixed by *routing*, not by detecting less (recall is the whole safety claim).
  ...
  - Demo ledger 12 → 21 grants across 8 gap archetypes; `docs/sample-rights-ledger.csv` 5 → 14 rows. Demo scene e1 relabelled to the description the model actually produced, so demo mode shows the promotion end to end. **262 tests, smoke green.**
...
- **Depiction-context signal.** The litigation record says brand risk turns on *how* a mark is shown, not that it is shown. Gemini can report disparagement / implied endorsement from footage already sent, at zero extra call cost. Not yet in `SCAN_PROMPT`.
...
- Tests: `.venv/bin/pytest -q` (262) · Smoke: `bash scripts/smoke.sh` · Frontend: `cd webapp && npm run build`
